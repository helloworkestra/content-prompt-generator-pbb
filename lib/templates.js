...
export const HOOK_COMBOS = [
  'Pain of invisibility + Trigger of realization',
  'Pain of wasted effort + Trigger of loss aversion',
  'Pain of uncertainty + Trigger of identity shift',
  'Pain of fear + Trigger of discomfort',
  'Pain of comparison + Trigger of contrast',
  'Pain of stagnation + Trigger of time sensitivity',
];

// Rotates HOOK_COMBOS based on how many days already exist. Auto-fill only.
export function hookComboForIndex(existingCount) {
  const i = ((existingCount % HOOK_COMBOS.length) + HOOK_COMBOS.length) % HOOK_COMBOS.length;
  return HOOK_COMBOS[i];
}
