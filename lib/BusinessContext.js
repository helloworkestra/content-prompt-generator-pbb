...
import { seedPortraitsForBusiness } from './portraitDefaults';
...
export function BusinessProvider({ children }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (list.length === 0) {
        ...
        await seedPortraitsForBusiness(created.id);
        list = [created];
      }
      setBusinesses(list);

      // Retroactive backfill: any business with zero portrait variations
      // (created before auto-seeding existed) gets the default set now.
      // seedPortraitsForBusiness is idempotent, so a stray retry is harmless.
      const { data: countRows } = await supabase
        .from('portrait_variations')
        .select('business_id');
      const seeded = new Set((countRows || []).map((r) => r.business_id));
      for (const b of list) {
        if (!seeded.has(b.id)) {
          // eslint-disable-next-line no-await-in-loop
          await seedPortraitsForBusiness(b.id);
        }
      }
  ...
  const addBusiness = useCallback(async (name) => {
    ...
    await seedPortraitsForBusiness(data.id);
